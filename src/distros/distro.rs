...
/// Returns a tuple: (list of metadata, bool indicating if data was served from cache).
///
/// # Parameter `audit`
/// Ketika `true`:
///   - Pesan `[DEBUG]` yang biasanya muncul hanya di stderr sekarang juga
///     dicetak dengan prefix `[AUDIT]` ke stdout.
///   - Output mentah dari lxc-download diteruskan via Stdio::inherit.
pub async fn get_lxc_distro_list(audit: bool) -> (Vec<DistroMetadata>, bool) {
    ...
    if cache_exists && is_cache_fresh(GLOBAL_CACHE).await && !Path::new(LOCK_FILE).exists() {
        if let Ok(content) = fs::read_to_string(GLOBAL_CACHE).await {
            if audit {
                println!("[AUDIT] Serving distribution list from cache: {}", GLOBAL_CACHE);
            }
    ...
    loop {
            ...
            .await
        {
            ...
            Err(_) => {
                // CHECK STALE LOCK: Remove the lock if it is older than 60 seconds
    ...
    if audit {
        println!("[AUDIT] Fetching distribution list from lxc-download — raw output follows:");
    }

    let output = if audit {
        // ── AUDIT MODE: teruskan stderr ke terminal (stdout tetap di-capture
        //    karena kita butuh parse hasilnya)
        Command::new("sudo")
    ...
    let result = match output {
        // [CRITICAL FIX]: Check if stdout contains "Distribution" instead of relying on exit code 0
        Ok(out)
        ...
        Ok(out) => {
            // Hanya tampilkan debug di audit mode. Di mode normal ini tidak dicetak
            // agar tidak merusak tampilan spinner.
            if audit {
                println!("[AUDIT] Main script failed. Stderr: {}", String::from_utf8_lossy(&out.stderr));
            } else {
            ...
            // FALLBACK PROTOCOL
            if audit {
                println!("[AUDIT] Trying fallback via lxc-create --list — raw output follows:");
            }

            let fallback = if audit {
                Command::new("sudo")
                    .args(&[
                        "-n", "-H", "lxc-create", "-n", "MELISA_PROBE_UNUSED",
                        "-t", "download", "--", "--list",
                    ])
                    .stderr(Stdio::inherit())
                    .output()
            ...
            if let Ok(fb_out) = fallback {
                ...
                if !content.is_empty()
                    && (content.contains("Distribution") || content.contains("DIST"))
                {
                    let _ = fs::write(GLOBAL_CACHE, content.to_string()).await;
                    let _ = Command::new("sudo")
                        .args(&["chmod", "666", GLOBAL_CACHE])
                    ...
                    if audit {
                        println!(
                            "[AUDIT] Fallback failed. Stderr: {}",
                            String::from_utf8_lossy(&fb_out.stderr)
                        );
                    } else {
                        eprintln!(
                            "[DEBUG] Fallback failed. Error: {}",
                            String::from_utf8_lossy(&fb_out.stderr)
                        );
                    }
                }
        ...
        Err(e) => {
            if audit {
                println!("[AUDIT] Failed to execute lxc-download command. Error: {}", e);
            } else {
